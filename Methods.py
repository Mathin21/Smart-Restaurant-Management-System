...
def total():
    ...
    Latte, Espresso, Iced_latte, SexOn, Cappucino = 10,7,35,20,20
    Frappé, Doppio, Iced_cap, Mojito = 20,20,20,40
    Cheese, Red, Pasta, lasagna, Carnitas = 30,30,40,20,15
    Cheeseburger, Layered, choc, Reuben = 20,50,45,45
    ...
    db_del()
    if g.var1.get() == 1: 
        db_insert_ml("Filter Cofee",l)
    if g.var2.get() == 1: 
        db_insert_ml("Tea",e)
    if g.var3.get() == 1: 
        db_insert_ml("Cold Coffe",il)
    if g.var4.get() == 1: 
        db_insert_ml("Thumps Up",v)
    if g.var5.get() == 1: 
        db_insert_ml("7up",c)
    if g.var6.get() == 1: 
        db_insert_ml("Sprite",af)
    if g.var7.get() == 1: 
        db_insert_ml("Doppio",am)
    if g.var8.get() == 1: 
        db_insert_ml("coca-cola",ic)
    if g.var9.get() == 1: 
        db_insert_ml("oreoms",im)
    if g.var10.get() == 1: 
        db_insert_ml("Idly",sc)
    if g.var11.get() == 1: 
        db_insert_ml("Mysore Bajji",su)
    if g.var12.get() == 1: 
        db_insert_ml("Dosa",j)
    if g.var13.get() == 1: 
        db_insert_ml("Samosa",w)
    if g.var14.get() == 1: 
        db_insert_ml("Veg puff",lc)
    if g.var15.get() == 1: 
        db_insert_ml("eggpuff",k)
    if g.var16.get() == 1: 
        db_insert_ml("Meals",ck)
    if g.var17.get() == 1: 
        db_insert_ml("Fried Rice",q)
    if g.var18.get() == 1: 
        db_insert_ml("Manchuria",p)
    #============
...
def recipt():
    ...
    g.listbox.insert("end", "                                                 ")
    if g.var1.get() == 1: 
        g.listbox.insert("end", "     Filter Cofee                                               {}                                            {}".format(g.Latte_Entry.get(), total_tuple[5]))
    if g.var2.get() == 1: 
        g.listbox.insert("end", "     Tea                                          {}                                            {}".format(g.Espresso_Entry.get(), total_tuple[6]))
        #db_insert("Tea",total_tuple[6])
    if g.var3.get() == 1: 
        g.listbox.insert("end", "     Cold Cofee                                         {}                                            {}".format(g.Iced_latte_Entry.get(), total_tuple[7]))
        #db_insert("Cold Coffe",total_tuple[7])
    if g.var4.get() == 1: 
        g.listbox.insert("end", "     Thumps Up                           {}                                            {}".format(g.SexOn_Entry.get(), total_tuple[8]))
        #db_insert("Thumps Up",total_tuple[8])
    if g.var5.get() == 1: 
        g.listbox.insert("end", "     7up                                     {}                                            {}".format(g.Cappucino_Entry.get(), total_tuple[9]))
        #db_insert("7up",total_tuple[9])
    if g.var6.get() == 1: 
        g.listbox.insert("end", "     Sprite                                            {}                                            {}".format(g.Frappé_Entry.get(), total_tuple[10]))
        #db_insert("Sprite",total_tuple[10])
    if g.var7.get() == 1: 
        g.listbox.insert("end", "     Doppio                                            {}                                            {}".format(g.Doppio_Entry.get(), total_tuple[11]))
        #db_insert("Coca-Cola",total_tuple[11])
    if g.var8.get() == 1: 
        g.listbox.insert("end", "     Coca-Cola                            {}                                            {}".format(g.Iced_cap_Entry.get(), total_tuple[12]))
        #db_insert("Oreoms",total_tuple[12])
    if g.var9.get() == 1: 
        g.listbox.insert("end", "      oreo milk shake                                            {}                                            {}".format(g.Mojito_Entry.get(), total_tuple[13]))
        #db_insert("Idly",total_tuple[13])
    if g.var10.get() == 1: 
        g.listbox.insert("end", "    Idly                                     {}                                           {}".format(g.Cheese_Entry.get(), total_tuple[14]))
        #db_insert("Mysorebajji",total_tuple[14])
    if g.var11.get() == 1: 
        g.listbox.insert("end", "    Mysoe_Bajji                                {}                                          {}".format(g.Red_Entry.get(), total_tuple[15]))
        #db_insert("Dosa",total_tuple[15])
    if g.var12.get() == 1: 
        g.listbox.insert("end", "    Dosa                              {}                                          {}".format(g.Pasta_Entry.get(), total_tuple[16]))
        #db_insert("Samosa",total_tuple[16])
    if g.var13.get() == 1: 
        g.listbox.insert("end", "    Samosa                          {}                                          {}".format(g.lasagna_Entry.get(), total_tuple[17]))
        #db_insert("Veg puff",total_tuple[17])
    if g.var14.get() == 1: 
        g.listbox.insert("end", "    Veg Puff                                {}                                          {}".format(g.Carnitas_Entry.get(), total_tuple[18]))
        #db_insert("Non Veg Puff",total_tuple[18])
    if g.var15.get() == 1: 
        g.listbox.insert("end", "   Non Veg Puff                                   {}                                          {}".format(g.Cheeseburger_Entry.get(), total_tuple[19]))
        #db_insert("Meals",total_tuple[19])
    if g.var16.get() == 1: 
        g.listbox.insert("end", "    Meals                     {}                                          {}".format(g.Layered_Entry.get(), total_tuple[20]))
        #db_insert("Noodles",total_tuple[20])
    if g.var17.get() == 1: 
        g.listbox.insert("end", "    Fried Rice                    {}                                          {}".format(g.choc_Entry.get(), total_tuple[21]))
        #db_insert("Fried Rice",total_tuple[21])
    if g.var18.get() == 1: 
        g.listbox.insert("end", "    Manchuria                            {}                                          {}".format(g.Reuben_Entry.get(), total_tuple[22]))
        #db_insert("Manchuria",total_tuple[22])
